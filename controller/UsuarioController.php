...
class UsuarioController
{
    ...
    public function validar()
    {
        if (isset($_POST['txtapodo'], $_POST['txtnombre'])) {
            ...
            $registros = $this->modelo->GetByNickName($txtapodo);

         if (empty($registros[0])) {
            $this->modelo->CrearSessionUsuario($txtapodo, $txtnombre);
            $this->modelo->Insert($txtapodo, $txtnombre);
            
         } else {
            $apodo = $registros[0]->Nickname;
            $nombre = $registros[0]->NombreCompleto;
            $this->modelo->CrearSessionUsuario($apodo, $nombre);
         }
